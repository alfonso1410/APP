{{-- resources/views/components/actividades/tabla-resumen.blade.php --}}
@props([
...
<div x-show="tablaCargada" class="space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-2">
            ...
            <template x-if="!periodoCerrado && hayDesincronizados">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-amber-800 bg-amber-50 border border-amber-200 rounded-md">
                    ...
                    <span>Hay calificaciones modificadas sin sincronizar</span>
                </span>
            </template>
        </div>
    ...
    <div class="bg-white rounded-md border border-[#E2E6EC] overflow-hidden shadow-sm">
        <div class="px-4 py-3 border-b border-[#E2E6EC] flex flex-wrap items-center justify-between gap-3 bg-[#F7F9FC]">
            <div>
                ...
                <p class="text-xs text-[#7A8AA0] mt-0.5">Vista resumen de actividades y promedio del criterio actual</p>
            </div>
                ...
                <thead>
                    <tr class="bg-[#F7F9FC] text-[#4A5A78] border-b border-[#E2E6EC] font-semibold text-xs uppercase">
                        ...
                        <template x-for="act in getActividadesDelCriterioActivo()" :key="act.id">
                            <th class="py-3 px-3 text-center border-l border-[#E2E6EC] min-w-[125px] group/col relative">
                                <div class="flex items-center justify-between gap-1">
                                    <div class="flex items-center gap-1 min-w-0">
                                        <span class="font-bold text-[#1A2332] truncate text-xs" x-text="act.nombre" :title="act.nombre"></span>
                                        
                                        <!-- BOTÓN CON ÍCONO (i) QUE ABRE EL MODAL -->
                                        <template x-if="act.descripcion || act.descripcion_actividad || act.detalle">
                                            <button type="button" @click="mostrarInfoActividad(act)" class="text-[#7A8AA0] hover:text-[#2E3E5C] transition-colors" title="Ver descripción">
                                ...
                                <template x-if="act.desincronizada">
                                    <div class="mt-1 text-[9px] font-bold text-[#B7791F] bg-amber-50 px-1.5 py-0.2 rounded inline-block">Editado</div>
                                </template>
                            </th>
                        </template>
                                ...
                                <td class="py-2 px-3 text-center border-l border-[#E2E6EC]">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <template x-if="getCalificacion(alumno, act.id).observaciones">
                                            <span class="w-1.5 h-1.5 rounded-full bg-[#2E3E5C]" :title="getCalificacion(alumno, act.id).observaciones"></span>
                                        </template>
                                    </div>
                                </td>
                            </template>
    ...
    <div x-show="alumnosFiltrados.length === 0" class="bg-white rounded-md p-8 border border-[#E2E6EC] text-center text-xs text-[#7A8AA0]">
